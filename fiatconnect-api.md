...
Depending on the type of key, the `key` field follows some validation rules. It MUST be an email, [CPF](https://en.wikipedia.org/wiki/CPF_number), mobile number (including area code) or a random (32 alphanumeric characters) key respectively. A random key is generated by bank.
