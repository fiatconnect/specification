...
Otherwise, if `keyType` is `RANDOM`, `key` MUST be a string of 36 characters consisting only of numbers 0-9, [ISO basic Latin alphabetic](https://en.wikipedia.org/wiki/ISO_basic_Latin_alphabet) letters, or [hyphen-minus](https://en.wikipedia.org/wiki/Hyphen-minus). If `keyType` is `RANDOM`, `key` represents a random key generated by the user's bank.
