...
To configure URLs on providers, provider must support a `POST` request to `/webhooks` endpoint.
...
* `url`: {`string`} [REQUIRED]
  - The webhook url where the status request should be posted by providers.
* `eventType`: ['WebhookEventTypeEnum'] [REQUIRED]
  - The event types for which the providers must send the webhook requests. 

##### 5.2.3. Responses

###### 5.2.3.1. HTTP `200`

On a successfully initiated webhook request, the server MUST respond with an HTTP `200` status code.

###### 5.2.3.2 HTTP `400`

If the webhook request parameters are invalid, the server MUST
return an HTTP `400` error code, along with the following response body:
...
###### 5.2.3.3. Idempotency Key Errors
