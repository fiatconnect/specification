...
Depending on the type of key, the `key` field follows some validation rules. It MUST be an email, [CPF](https://en.wikipedia.org/wiki/CPF_number), mobile number (including area code) or a random (32 alphanumeric characters) key respectively. A random key is generated by bank. The mobile number follows the [Brazilian telephone number standard](https://pt.wikipedia.org/wiki/N%C3%BAmeros_de_telefone_no_Brasil). Example: DD9XXXXXXXX or 55DD9XXXXXXXX
